...
# neweraofcoding Brand Colors
DR_BLUE=$'\033[38;5;27m'
DR_CYAN=$'\033[38;5;51m'
DR_PURPLE=$'\033[38;5;93m'

clear

echo "${DR_BLUE}${BOLD_TEXT}"
echo "╔═══════════════════════════════════════════════════╗"
echo "║                                                   ║"
echo "║           neweraofcoding Cloud Tutorials            ║"
echo "║                                                   ║"
echo "║  Comprehensive GCP Learning Resources             ║"
echo "║  YouTube: https://youtube.com/@neweraofcoding.5460    ║"
echo "║                                                   ║"
echo "╚═══════════════════════════════════════════════════╝"
echo "${RESET_FORMAT}"

echo
echo "${DR_CYAN}${BOLD_TEXT}===================================${RESET_FORMAT}"
echo "${DR_CYAN}${BOLD_TEXT}🚀     INITIATING EXECUTION     🚀${RESET_FORMAT}"
echo "${DR_CYAN}${BOLD_TEXT}===================================${RESET_FORMAT}"
echo

echo "${DR_BLUE}${BOLD_TEXT}🛠️ Attempting to determine the default Google Cloud Zone...${RESET_FORMAT}"
...
echo
echo "${DR_BLUE}${BOLD_TEXT}🛠️ Attempting to determine the default Google Cloud Region...${RESET_FORMAT}"
...
if [ -z "$REGION" ]; then
  ...
  if [ -n "$ZONE" ]; then
    echo "${DR_BLUE}${BOLD_TEXT}⚙️ Trying to derive region from the provided zone: $ZONE${RESET_FORMAT}"
    REGION="${ZONE%-*}"
...
echo "${DR_BLUE}${BOLD_TEXT}🆔 Fetching your Google Cloud Project ID...${RESET_FORMAT}"
export PROJECT_ID=$(gcloud config get-value project)
echo "${GREEN_TEXT}${BOLD_TEXT}✅ Project ID identified as: $PROJECT_ID${RESET_FORMAT}"
echo

echo "${DR_BLUE}${BOLD_TEXT}⚙️ Configuring default compute region to $REGION...${RESET_FORMAT}"
gcloud config set compute/region $REGION
echo "${GREEN_TEXT}${BOLD_TEXT}👍 Default compute region configured.${RESET_FORMAT}"
echo

echo "${DR_BLUE}${BOLD_TEXT}🛠️ Enabling necessary Google Cloud services. This might take a moment...${RESET_FORMAT}"
...
echo
echo "${DR_BLUE}${BOLD_TEXT}🏗️ Initiating creation of GKE clusters (test, staging, prod) asynchronously...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}📋 Displaying initial list of GKE clusters and their statuses...${RESET_FORMAT}"
gcloud container clusters list --format="csv(name,status)"
echo

echo "${DR_BLUE}${BOLD_TEXT}📦 Creating Artifact Registry repository 'web-app' for Docker images...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🚚 Preparing project files: Navigating to home, cloning repository, and checking out specific version...${RESET_FORMAT}"
cd ~/
...
echo "${DR_BLUE}${BOLD_TEXT}📝 Generating Skaffold configuration from template...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🛠️ Ensuring Cloud Build GCS bucket (gs://${PROJECT_ID}_cloudbuild) exists...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🧱 Building the application using Skaffold. This may take some time...${RESET_FORMAT}"
cd web
...
echo "${DR_BLUE}${BOLD_TEXT}ℹ️ Using 'web/artifacts.json' directly for Cloud Deploy release.${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🖼️ Listing Docker images in the Artifact Registry repository...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}⚙️ Configuring default Cloud Deploy region to $REGION...${RESET_FORMAT}"
gcloud config set deploy/region $REGION
echo "${GREEN_TEXT}${BOLD_TEXT}👍 Cloud Deploy region configured.${RESET_FORMAT}"
echo

echo "${DR_BLUE}${BOLD_TEXT}📜 Setting up the Cloud Deploy delivery pipeline by copying and applying configuration...${RESET_FORMAT}"
cp clouddeploy-config/delivery-pipeline.yaml.template clouddeploy-config/delivery-pipeline.yaml
gcloud beta deploy apply --file=clouddeploy-config/delivery-pipeline.yaml
echo "${GREEN_TEXT}${BOLD_TEXT}✅ Delivery pipeline configuration applied.${RESET_FORMAT}"
echo

echo "${DR_BLUE}${BOLD_TEXT}📄 Describing the 'web-app' delivery pipeline details...${RESET_FORMAT}"
gcloud beta deploy delivery-pipelines describe web-app
echo

echo "${DR_BLUE}${BOLD_TEXT}⏳ Monitoring GKE cluster statuses. Waiting for all clusters to be in 'RUNNING' state...${RESET_FORMAT}"
while true; do
...
echo "${DR_BLUE}${BOLD_TEXT}🔑 Fetching GKE cluster credentials and renaming kubectl contexts for easier access...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🏠 Applying Kubernetes namespace 'web-app' to all contexts...${RESET_FORMAT}"
...
echo "${DR_BLUE}${BOLD_TEXT}🎯 Configuring Cloud Deploy targets for each context...${RESET_FORMAT}"
for CONTEXT in ${CONTEXTS[@]}
do
...
echo
echo "${DR_BLUE}${BOLD_TEXT}🚀 Creating Cloud Deploy release 'web-app-001'...${RESET_FORMAT}"
...
if [ $RELEASE_CREATION_STATUS -eq 0 ]; then
  ...
  echo "${DR_BLUE}${BOLD_TEXT}Cloud Deploy will now automatically roll out this release to the first target ('test').${RESET_FORMAT}"
else
...
echo "${DR_BLUE}${BOLD_TEXT}⏳ Waiting for the initial rollout to 'test' target to complete...${RESET_FORMAT}"
while true; do
...
if [ "$test_rollout_succeeded" = true ]; then
  echo "${DR_BLUE}${BOLD_TEXT}🔬 Switching to 'test' Kubernetes context and verifying deployed resources...${RESET_FORMAT}"
  kubectx test
  kubectl get all -n web-app
  echo

  echo "${DR_BLUE}${BOLD_TEXT}➡️ Promoting release 'web-app-001' to the 'staging' target...${RESET_FORMAT}"
  ...
  echo "${DR_BLUE}${BOLD_TEXT}⏳ Waiting for the rollout to 'staging' target to complete...${RESET_FORMAT}"
  while true; do
  ...
  if [ "$staging_rollout_succeeded" = true ]; then
    echo "${DR_BLUE}${BOLD_TEXT}➡️ Promoting release 'web-app-001' to the 'prod' target (this will require approval)...${RESET_FORMAT}"
    gcloud beta deploy releases promote \
    --delivery-pipeline web-app \
    --release web-app-001 \
    --quiet
    ...
    echo "${DR_BLUE}${BOLD_TEXT}⏳ Waiting for the rollout to 'prod' to reach 'PENDING_APPROVAL' state...${RESET_FORMAT}"
    while true; do
    ...
    if [ "$prod_rollout_pending_approval" = true ]; then
      ...
      if [ -n "$prod_rollout_name" ]; then
        echo "${DR_BLUE}${BOLD_TEXT}✅ Approving rollout '$prod_rollout_name' for 'prod' target...${RESET_FORMAT}"
        ...
        echo "${DR_BLUE}${BOLD_TEXT}⏳ Waiting for the rollout to 'prod' target to complete after approval...${RESET_FORMAT}"
        while true; do
          status=$(gcloud beta deploy rollouts list --delivery-pipeline web-app --release web-app-001 --filter="targetId=prod" --format="value(state)" | head -n 1)
        ...
        if [ "$prod_rollout_succeeded" = true ]; then
          echo "${DR_BLUE}${BOLD_TEXT}🔬 Switching to 'prod' Kubernetes context and verifying deployed resources...${RESET_FORMAT}"
          kubectx prod
          kubectl get all -n web-app
        else
...
echo
echo "${DR_PURPLE}${BOLD_TEXT}💖 Enjoyed this tutorial? Consider subscribing to neweraofcoding Cloud Tutorials! 👇${RESET_FORMAT}"
echo "${DR_BLUE}${BOLD_TEXT}${UNDERLINE_TEXT}https://youtube.com/@neweraofcoding.5460${RESET_FORMAT}"
echo
