...
echo "${BLUE}${BOLD}https://www.youtube.com/@neweraofcoding/videos${RESET}"
echo
...
while true; do
    ...
    if [ $? -eq 0 ]; then
        echo "${YELLOW}${BOLD}Job has completed successfully. Now just wait for succeeded${RESET} ${BLUE}${BOLD}https://www.youtube.com/@neweraofcoding${RESET}"
        break
    else
        echo "${YELLOW}${BOLD}Job retrying. Please like, share and subscribe to neweraofcoding Cloud Tutorials${RESET} ${BLUE}${BOLD}https://www.youtube.com/@neweraofcoding${RESET}"
        sleep 10
    fi
done
